...
- AGENT $id=interview-day $input=self/endurance.toon.md $output=pedagogy/pedagogy-cpu.fu.md $when=after interview-graph $prompt=python cron/interview-anchor.py --day. Date is today Europe/Berlin unless --date. LOAD is remaining after planned daily external (德语助手 wordschatz). First DO is the open PROBE if ANSWER is empty. Then that many interview titles on north vertices. Print the slice in order. Stop. Wait for the human. Do not rewrite a DAY that already has DO.
...
- AGENT $id=interview-external $input=self/endurance.toon.md $output=self/endurance.toon.md $when=human-external $prompt=python cron/interview-anchor.py --external --id de-assistant-wordschatz. Log one Wordschatz unit. Do not invent a word count. Do not diagnose DSB.
...
- RULE IntellectualLoad is the bounded-study vertex. External Wordschatz (德语助手) is one unit per session. DAY LOAD is remaining after planned daily external. Sore is a stop signal like scapular pain in training.toon.md. Hypothesis DSB is the human's name for the sore. Cite Nature 2024 only as the mouse DDR, with the contradiction that activity-induced enhancer breaks repair in minutes and persistent extranuclear fragments are a different class.
