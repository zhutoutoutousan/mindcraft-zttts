...
- AGENT $id=schedule-harvest $input=CPU.md $output=schedule/harvest.toon.md $prompt=Do not stop at slugs. For every dated candidate read CPU.md inflow/STATE.md inflow/inflow.fu.md .private/*.fu.md raw/*.fu.md. Then browse the open web until the row has a human name plus WHAT WHERE WHEN WHY and a SOURCE url. Write those facts into schedule/enrich.toon.md. Never invent a clock time. If listings disagree, cite both and pick one. If a street number is absent, leave it absent. Do not rename KI and Coffee Mitte to Berlin Capital Club KI-LOUNGE. That is a different evening event. Skip paper dumps with no sitting date. Skip Goethe module durations with no exam day. Skip invite-only rooms the human cannot enter. Then python cron/schedule-harvest.py --apply so harvest and the month fu merge the overlay.

- AGENT $id=schedule-ics $input=schedule/harvest.toon.md $output=schedule/calendar.ics $when=human-asks $prompt=python cron/schedule-harvest.py --ics. Google Calendar import. SUMMARY is the human name never a slug. LOCATION is a mappable WHERE. DESCRIPTION is what the human reads in the popup: what it is, where, when, and why it matters to the job-seeking AI agent engineer in self/goals.toon.md. Never dump source or status into DESCRIPTION. TZ Europe/Berlin. Do not run unless the human asked for ics.

- RULE slug titles like ki croissants potsdam are unfinished. Apply is not done until enrich overlay has WHAT WHERE WHEN WHY. ICS DESCRIPTION is the Google popup. If the popup would show only source plus status, the ics job failed.
