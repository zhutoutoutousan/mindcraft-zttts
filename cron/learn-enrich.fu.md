...
- RULE every tick read self/goals.toon.md. focus chooses PLAN. north fields merge into pick order. Age and language counts stay particulars. Sixteen language names: pedagogy/_learn/polyglot/horizon.toon.md. Do not invent CEFR beyond those bands. Do not invent lebenslauf SPRACHEN lines.
...
- AGENT $id=learn-baseline $input=pedagogy/_learn/baseline-probes.toon.md $output=tmp/pedagogy/baseline.html $prompt=python cron/learn-enrich.py --baseline-serve. 摸底 in tmp. Port 8765 then 8777. Open in Chrome or Edge, not the Cursor panel. Speak asks getUserMedia then Web Speech into the box. Async. Do not invent ANSWER. Do not assess. Do not add Chinese as native. Do not spawn janitor --apply.
